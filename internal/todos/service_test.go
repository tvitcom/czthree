...
func (m mockRepository) Get(ctx context.Context, id string) (entity.todos, error) {
	for _, item := range m.items {
		if item.ID == id {
			return item, nil
		}
	}
	return entity.todos{}, sql.ErrNoRows
}
...
func (m mockRepository) Query(ctx context.Context, offset, limit int) ([]entity.todos, error) {
	return m.items, nil
}

func (m *mockRepository) Create(ctx context.Context, album entity.todos) error {
	if album.Name == "error" {
		return errCRUD
	}
...
func (m *mockRepository) Update(ctx context.Context, album entity.todos) error {
	if album.Name == "error" {
		return errCRUD
	}
