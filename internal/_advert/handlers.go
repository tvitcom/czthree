...
// println("FORM:",form.TodoId, form.SignerUA, form.SignerScreen, form.SignerLangs, form.SignerTime)
    ...
    if result {
        author, err := res.agregator.GetUserByTodoId(c.UserContext(), form.TodoId)
        if err != nil {
            return c.Status(500).JSON(&fiber.Map{
                "ok": result,
...
func (res resource) handlerDownloadMyResume(c *fiber.Ctx) error {
    return c.Download("./data/CV_Backend_RU.pdf", "Filippov_Roman_GoPHPJSLinux_cv.pdf");
}